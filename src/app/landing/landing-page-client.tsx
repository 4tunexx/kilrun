...
/** Branding fields hydrated from the server so first paint uses admin logos. */
export type LandingInitialSettings = {
...
function buildHeroSlides(s: LandingInitialSettings): { src: string; alt: string }[] {
...
export default function LandingPageClient({
  ...
  initialSettings: LandingInitialSettings;
}) {
  const autoplayPlugin = useRef(
    Autoplay({ delay: 3000, stopOnInteraction: true })
  );
  const [headerTitle, setHeaderTitle] = useState(
    () => initialSettings.headerTitle?.trim() || 'Welcome to Kilrun'
  );
  const [headerSubtitle, setHeaderSubtitle] = useState(
    () =>
      initialSettings.headerSubtitle?.trim() ||
      'The ultimate deathrun experience. Compete, conquer, and climb the ranks.'
  );
  const [bgUrl, setBgUrl] = useState(() =>
    resolveHubBackground(initialSettings.backgroundUrl)
  );
  const [heroSlides, setHeroSlides] = useState(() =>
    buildHeroSlides(initialSettings)
  );
  const [showLandingSlider, setShowLandingSlider] = useState(
    () => parseHubChrome(initialSettings.hubChromeJson).showLandingSlider
  );
  const [markLogoUrl, setMarkLogoUrl] = useState(
    () => initialSettings.logoUrl ?? ''
  );
  const [headerLogoUrl, setHeaderLogoUrl] = useState(
    () => initialSettings.headerLogoUrl ?? ''
  );
  const [headerLogoStyle, setHeaderLogoStyle] = useState<HeaderLogoStyle>(() =>
    normalizeHeaderLogoStyle(
      initialSettings.headerLogoStyle ?? DEFAULT_HEADER_LOGO_STYLE
    )
  );
  ...
  const applyBranding = (s: LandingInitialSettings) => {
    ...
    if (s.hubChromeJson !== undefined) {
      setShowLandingSlider(parseHubChrome(s.hubChromeJson).showLandingSlider);
    }
    if (
      s.landingHeroSlides !== undefined ||
      s.landingHeroImage !== undefined
    ) {
      setHeroSlides(buildHeroSlides(s));
    }
