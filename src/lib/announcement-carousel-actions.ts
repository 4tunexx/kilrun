  ...
  return withPrismaRetry(async () => {
    ...
    try {
      const settings = await prisma.siteSettings.findFirst({
        ...
        select: { announcementCarouselJson: true },
      });
      raw = settings ? { announcementCarouselJson: settings.announcementCarouselJson } : {};
    } catch (error) {
      console.error('[announcement-carousel] failed to load site settings', error);
    }
    ...
    await Promise.all(
      config.types.map(async (type) => {
        try {
          ...
          console.error(`[announcement-carousel] failed to load type ${type}`, error);
        }
      })
    );
    ...
    if (items.length === 0) {
      try {
        ...
        console.error('[announcement-carousel] fallback news load failed', error);
      }
    }
